...
// here's the brand new method that automatically cancels the database once block is left
...
One more example i came up with is to enable certain mechanism only for a given block scope. In this scenario we don't actually use the `using` keyword the way it was designed, but can leverage the fact the `[Symbol.dispose]` is being called right before the block is "orphaned".
...
## How funny the joke about react `"use using"` directive actually is?
...
There was a meme posted on X about Next.js acquiring yet another directive called "use using"; while it's quite funny i think we aren't in much danger there, and here's why:

### Component function lifetime

If you've ever written a react component you know that it's nothing else than a function that can use some hooks along the way and return a final node that should be rendered; most of the times it's wrapped with a charming jsx pattern that just tries to imitate you write an html-alike code.
  ...
  while (isMounted) {
    yield (
      <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
        ...
        <p style={{ fontSize: '24px' }}>Count: {state.get('count')}</p>
